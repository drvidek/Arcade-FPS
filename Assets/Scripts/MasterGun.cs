...
public abstract class MasterGun : MonoBehaviour
{
    ...
    virtual public void ApplyBulletProperties(Bullet bullet, Vector3 dir, Vector3 drift, int count)
    {
        ...
        Vector3 horScatter = bullet.transform.TransformDirection(Vector3.left) * Random.Range(-_finalSpread, _finalSpread) * (float)(count);   //scatter reduced to 0 for a scoped
        Vector3 verScatter = bullet.transform.TransformDirection(Vector3.up) * Random.Range(-_finalSpread, _finalSpread) * (float)(count);     //hitscan single shot
        Vector3 newDir = dir + horScatter + verScatter;
        LayerMask mask = tag == "Player" ? LayerMask.NameToLayer("Target"): LayerMask.NameToLayer("Player");
        bullet.Initialise(newDir, drift, _shotSpeed, _shotPower, _shotSize, mask);
    }
